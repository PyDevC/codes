// RUN: toy-opt %s --loop-invariant-code-motion > %t
// RUN: FileCheck %s < %t
...
module {
    ...
    func.func @test_loop_invariant_code_motion() -> (!poly.poly<10>){
        ...
        // CHECK: poly.mul
        %ret-val = affine.for %i = 0 to 100 iter_args(%sum_iter = %p0) -> !poly.poly<10> {
            // The poly.mul should be out of the loop
            // CHECK-NOT: poly.mul
